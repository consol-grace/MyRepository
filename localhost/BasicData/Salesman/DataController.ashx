...
public class DataController : DIYGENS.COM.BASECLASS.ControllerBase
{
    ...
    private void list()
    {
        DataTable dt = this.FactoryDAL(PageHelper.ConnectionStrings, "FW_BasicData_Sales_SP", new List<IFields>() { this.CreateIFields().Append("Option", "List")
    ...
    private void update()
    {
        bool bFlag = this.FactoryDAL(PageHelper.ConnectionStrings, "FW_BasicData_Sales_SP", new List<IFields>() { this.CreateIFields().Append("Option", "Update")
                ...
                .Append("Company",Request.Form["Company"].Trim()=="Select..."?"":Request.Form["Company"].Trim())
                .Append("USGroup",Request.QueryString["chkUSGroup"])

...
public partial class Triangle_CallNo : System.Web.UI.Page
{
    ...
    void DataBinder(string id)
    {
        if (id.Length > 0)
        {
            ...
            DataSet ds = dal.FactoryDAL(PageHelper.ConnectionStrings, "FW_CallShipmentForTR_SP", new List<IFields>() { dal.CreateIFields()
    ...
    protected void btnDetail_Click(object sender, EventArgs e)
    {
        ...
        if (ds != null && ds.Tables[0].Rows.Count > 0)
        {
            ...
            {
                if (ds.Tables[0].Rows[0][2].ToString() == "AIR")
                {
                    X.AddScript("window.open('../AirShipment/List.aspx?seed=" + ds.Tables[0].Rows[0][0].ToString() + "');");
                }
                else
                {
                    X.AddScript("window.open('../OceanShipment/List.aspx?seed=" + ds.Tables[0].Rows[0][0].ToString() + "');");
                }
            }
    ...
    private DataSet GetDs()
    {
        ...
        DataFactory dal = new DataFactory();
        ds = dal.FactoryDAL(PageHelper.ConnectionStrings, "FW_CallShipmentForTR_SP", new List<IFields>() { dal.CreateIFields()
    ...
    protected void btnLotlist_Click(object sender, EventArgs e)
    {
        ...
        if (ds != null && ds.Tables[0].Rows.Count > 0)
        {
            ...
            {
                sys = ds.Tables[0].Rows[0][2].ToString() == "AIR" ? "A" : "O";
            }

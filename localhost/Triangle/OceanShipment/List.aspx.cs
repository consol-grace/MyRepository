...
public partial class Triangle_OceanShipment_List : System.Web.UI.Page
{
    ...
    void DataBinder()
    {
        ...
        DataSet ds = dal.FactoryDAL(PageHelper.ConnectionStrings, "FW_Triangle_Ocean_SP", new List<IFields>() { dal.CreateIFields().Append("Option", "GetList").Append("tri_Seed", hidSeed.Text) }).GetList();
        if (ds != null && ds.Tables[0].Rows.Count > 0)
        {
            ...
            ControlBinder.pageTitleMsg(false, "Ocean Shipment:" + ds.Tables[0].Rows[0]["tri_LotNo"].ToString(), "<p>Status :  Edit  Ocean Shipment  of <span>" + ds.Tables[0].Rows[0]["tri_LotNo"].ToString() + "</span></p>", div_bottom);
        }
        else
        {
            ...
            ControlBinder.pageTitleMsg(false, "Ocean Shipment:New", "<p>Status :  New  Blank  Ocean Shipment </p>", div_bottom);
        }
    ...
    public void btnUpdateLotNo_Click()
    {
        ...
        string lotNo = ControlBinder.GetNewLotNo("OT", hidSeed.Text, etd);
        if (lotNo == "-1")
        {
        ...
        else if (lotNo != "0")
        {
            ...
            ControlBinder.pageTitleMsg(true, "Ocean Shipment:" + hidLotNo.Text, "<p class=\"success\">Status : Saved successfully ! ! ! </p>", div_bottom);
            btnUpdateLotNo.Hide();
        }
    }
    ...
    protected void btnSave_Click(object sender, DirectEventArgs e)
    {
        ...
        DataTable dt = dal.FactoryDAL(PageHelper.ConnectionStrings, "FW_Triangle_Ocean_SP", new List<IFields>() { dal.CreateIFields().Append("Option", "Update")
            ...
            .Append("code",FSecurityHelper.CurrentUserDataGET()[4].ToString()+"OT")
            .Append("tri_Remark",string.IsNullOrEmpty(txtMAWBRemark.Text) ? "" : txtMAWBRemark.Text.Trim().ToUpper())
        }).GetTable();
        ...
        //bool inv = dal.FactoryDAL(PageHelper.ConnectionStrings, "FW_Triangle_Ocean_SP", new List<IFields>() { dal.CreateIFields().Append("Option", "DeleteActive")
        ...
        if (dt != null)
        {
            ...
            //ControlBinder.UpdateCostData(hidSeed.Text, "OT", "");
            ControlBinder.pageTitleMsg(true, "Ocean Shipment:" + labLotNo.Text, "<p class=\"success\">Status : Record Saved with Lot No.  <span>" + labLotNo.Text + "</span>  </p>", div_bottom);
            if (dt != null && dt.Rows.Count > 0 && !string.IsNullOrEmpty(dt.Rows[0][0].ToString()))
            {
        ...
        {
            ControlBinder.pageTitleMsg(false, "Ocean Shipment:" + labLotNo.Text, "<p class=\"error\">Status :  Saved  failed ! ! !  </p>", div_bottom);
        }
    ...
    public void btnVoid_Click()
    {
        ...
        if (!VoidCheckAC.CheckisAC("OT", hidSeed.Text))
        {
        ...
        if (seedid != "")
        {
            ...
            {
                ControlBinder.pageTitleMsg(false, "Ocean Shipment:" + labLotNo.Text, "<p class=\"error\"> Status : Saving  failed  ! ! !   </p>", div_bottom);
            }
            DataBinder();
            ControlBinder.pageTitleMsg(true, "Ocean Shipment:" + labLotNo.Text, "<p>Status :  Edit  Ocean Shipment  of <span>" + labLotNo.Text + "</span></p>", div_bottom);
        }
    }

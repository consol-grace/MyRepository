...
public partial class OceanImport_OceanShipmentJobList_ReportFile : System.Web.UI.Page
{
    ...
    private void GetPrintMargin()
    {
        DataSet ds = dal.FactoryDAL(PageHelper.ConnectionStrings, "FW_BasicData_Print_SP", new List<IFields>() { dal.CreateIFields().Append("Option", "PrintMargin")
                 .Append("prt_STAT",FSecurityHelper.CurrentUserDataGET()[12]).Append("prt_ReportCode",Request.QueryString["type"]).Append("prt_sys","OI").Append("prt_PrinterName",CmbPrint.SelectedItem.Value)
        }).GetList();
        if (ds != null && ds.Tables[0].Rows.Count > 0)
        {

...
public partial class Triangle_OceanShipment_List : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!X.IsAjaxRequest)
        {
            ...
            ControlBinder.DateFormat(txtDeliveryDate);
    ...
    protected void Page_PreRender(object sender, EventArgs e)
    {
        if (!X.IsAjaxRequest)
        {
            if (!IsPostBack)
            {
                ...
                DataBinder();
                LockControl();
                //X.AddScript("showCompanyRemark('CmbShipperCode,CmbCarrierCode,CmbConsigneeCode', 'Shipper,Carrier,Consignee');");
            }
        }
    }
    ...
    public void DataBindPO()
    {
        ...
        if (dt != null)
        {
            txtCBM.Text = dt.Rows[0]["dod_CBM"].ToString();
        }
    }
    ...
    public void AddPOData(string rowID)
    {
        ...
        DataSet ds = dal.FactoryDAL(PageHelper.ConnectionStrings, "FW_Other_Trucking_SP", new List<IFields>() { dal.CreateIFields().Append("Option", "GetList").Append("tri_Seed", hidSeed.Text) }).GetList();
        ...
        if (dt != null && ds != null)
        {
            if (string.IsNullOrEmpty(dt.Rows[0]["dod_CBM"].ToString()))
            {
                ...
                {
                    txtCBM.Text = decimal.Parse(ds.Tables[0].Rows[0]["tri_CBM"].ToString()).ToString();
                }
            }
            else
            {
                if (string.IsNullOrEmpty(ds.Tables[0].Rows[0]["tri_CBM"].ToString()))
                {
                    txtCBM.Text = decimal.Parse(dt.Rows[0]["dod_CBM"].ToString()).ToString();
                }
                else
                {
                    txtCBM.Text = (decimal.Parse(dt.Rows[0]["dod_CBM"].ToString()) + decimal.Parse(ds.Tables[0].Rows[0]["tri_CBM"].ToString())).ToString();
                }
            }
    ...
    void DataBinder()
    {
        ...
        if (ds != null && ds.Tables[0].Rows.Count > 0)
        {
            ...
            txtDeliveryDate.Text = ds.Tables[0].Rows[0]["tri_ETA"].ToString();
            ...
            //txtPickName.Text = ds.Tables[0].Rows[0]["tri_Shipper"].ToString();
            //txtPickNameText.Text = ds.Tables[0].Rows[0]["tri_ShipperLine"].ToString();
            ...
            //txtDeliveryName.Text = ds.Tables[0].Rows[0]["tri_Consignee"].ToString();
            //txtDeliveryNameText.Text = ds.Tables[0].Rows[0]["tri_ConsigneeLine"].ToString();
            ...
            txtSO.Text = ds.Tables[0].Rows[0]["tri_SONo"].ToString();
            ...
            ControlBinder.pageTitleMsg(false, "Trucking Shipment:" + ds.Tables[0].Rows[0]["tri_LotNo"].ToString(), "<p>Status :  Edit  Trucking Shipment  of <span>" + ds.Tables[0].Rows[0]["tri_LotNo"].ToString() + "</span></p>", div_bottom);
        }
        else
        {
            ...
            ControlBinder.pageTitleMsg(false, "Trucking Shipment:New", "<p>Status :  New  Blank  Trucking Shipment </p>", div_bottom);
        }
    ...
    public void btnUpdateLotNo_Click()
    {
        ...
        string lotNo = ControlBinder.GetNewLotNo("TK", hidSeed.Text, etd);
        if (lotNo == "-1")
        {
            X.MessageBox.Alert("Status", "The cost of this LOT is transfered to AC, LOT NUMBER updating is aborted!").Show();
            ControlBinder.pageTitleMsg(true, "Trucking Shipment:" + hidLotNo.Text, "<p class=\"success\">Status : Saved failed ! ! ! </p>", div_bottom);
        }
        else if (lotNo != "0")
        {
            labLotNo.Text = lotNo;
            ...
            ControlBinder.pageTitleMsg(true, "Trucking Shipment:" + hidLotNo.Text, "<p class=\"success\">Status : Saved successfully ! ! ! </p>", div_bottom);
            btnUpdateLotNo.Hide();
        }
    }

    [DirectMethod]
    public void GetName(string obj, string code)
    {
        DataSet dsCom = dal.FactoryDAL(PageHelper.ConnectionStrings, "FW_Other_Trucking_SP", new List<IFields>() { dal.CreateIFields().
        Append("Option", "GetCompanyName").
        Append("code", code)}).GetList();
        if (dsCom != null && dsCom.Tables[0].Rows.Count > 0)
        {
            if (obj == "CmbShipperCode")
            {
                CmbShipperCode_name.Text = dsCom.Tables[0].Rows[0][0].ToString();
                CmbShipperCode1.Text = code;
                CmbShipperCode_name.Focus();
            }
            else if (obj == "CmbConsigneeCode")
            {
                CmbConsigneeCode_name.Text = dsCom.Tables[0].Rows[0][0].ToString();
                CmbConsigneeCode1.Text = code;
                CmbConsigneeCode_name.Focus();
            }
        }
    }
    ...
    protected void btnSave_Click(object sender, DirectEventArgs e)
    {
        ...
        object ETA = ControlBinder.getDate(string.IsNullOrEmpty(txtDeliveryDate.RawText.Trim()) ? DBNull.Value : (object)txtDeliveryDate.RawText.Trim());
        ...
        DataTable dt = dal.FactoryDAL(PageHelper.ConnectionStrings, "FW_Other_Trucking_SP", new List<IFields>() { dal.CreateIFields().Append("Option", "Update")
            ...
            //.Append("tri_Shipper",txtPickName.Text.Trim().ToUpper())
            //.Append("tri_ShipperLine",txtPickNameText.Text.Trim().ToUpper())
            //.Append("tri_Consignee",txtDeliveryName.Text.Trim().ToUpper())
            //.Append("tri_ConsigneeLine",txtDeliveryNameText.Text.Trim().ToUpper())
            .Append("tri_SONo",txtSO.Text.Trim().ToUpper())
            ...
            .Append("code",FSecurityHelper.CurrentUserDataGET()[4].ToString()+"TK")
        ...
        //bool inv = dal.FactoryDAL(PageHelper.ConnectionStrings, "FW_Other_Trucking_SP", new List<IFields>() { dal.CreateIFields().Append("Option", "DeleteActive")
        ...
        if (dt != null)
        {
            ...
            ControlBinder.pageTitleMsg(true, "Trucking Shipment:" + labLotNo.Text, "<p class=\"success\">Status : Record Saved with Lot No.  <span>" + labLotNo.Text + "</span>  </p>", div_bottom);
            if (dt != null && dt.Rows.Count > 0 && !string.IsNullOrEmpty(dt.Rows[0][0].ToString()))
            {
                ...
                if (type == "p")
                {
                    ...
                    .Append("DODBookingNo",dt.Rows[0][1]).Append("DODRowID",DODRowID).Append("Master", dt.Rows[0][0]).Append("House",DBNull.Value)}).Update();
                    #endregion
            ...
            else
            {
                ...
                #region updatePOBookingNo
                dal.FactoryDAL(PageHelper.ConnectionStrings, "FW_DeliveryOrder_Pool_SP", new List<IFields> { dal.CreateIFields().Append("Option", "UpdateBookingNO")
                    .Append("DODBookingNo",labLotNo.Text).Append("DODRowID",DODRowID).Append("Master",hidSeed.Text).Append("House",DBNull.Value)}).Update();
                #endregion
        ...
        {
            ControlBinder.pageTitleMsg(false, "Trucking Shipment:" + labLotNo.Text, "<p class=\"error\">Status :  Saved  failed ! ! !  </p>", div_bottom);
        }
    ...
    public void btnVoid_Click()
    {
        ...
        if (!VoidCheckAC.CheckisAC("TK", hidSeed.Text))
        {
        ...
        if (seedid != "")
        {
            ...
            {
                ControlBinder.pageTitleMsg(false, "Trucking Shipment:" + labLotNo.Text, "<p class=\"error\"> Status : Saving  failed  ! ! !   </p>", div_bottom);
            }
            DataBinder();
            ControlBinder.pageTitleMsg(true, "Trucking Shipment:" + labLotNo.Text, "<p>Status :  Edit  Trucking Shipment  of <span>" + labLotNo.Text + "</span></p>", div_bottom);
        }
    }

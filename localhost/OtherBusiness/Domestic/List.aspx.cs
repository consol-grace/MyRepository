...
public partial class Triangle_OceanShipment_List : System.Web.UI.Page
{
    ...
    void DataBinder()
    {
        ...
        DataSet ds = dal.FactoryDAL(PageHelper.ConnectionStrings, "FW_Other_Domestic_SP", new List<IFields>() { dal.CreateIFields().Append("Option", "GetList").Append("tri_Seed", hidSeed.Text) }).GetList();
    ...
    public void btnUpdateLotNo_Click()
    {
        ...
        string lotNo = ControlBinder.GetNewLotNo("DM", hidSeed.Text, etd);
        if (lotNo == "-1")
        {
            X.MessageBox.Alert("Status", "The cost of this LOT is transfered to AC, LOT NUMBER updating is aborted!").Show();
            ControlBinder.pageTitleMsg(true, "Domestic Shipment:" + hidLotNo.Text, "<p class=\"success\">Status : Saved failed ! ! ! </p>", div_bottom);
        ...
        else if (lotNo != "0")
        {
            ...
            ControlBinder.pageTitleMsg(true, "Domestic Shipment:" + hidLotNo.Text, "<p class=\"success\">Status : Saved successfully ! ! ! </p>", div_bottom);
            btnUpdateLotNo.Hide();
        }
    }
    ...
    protected void btnSave_Click(object sender, DirectEventArgs e)
    {
        ...
        DataTable dt = dal.FactoryDAL(PageHelper.ConnectionStrings, "FW_Other_Domestic_SP", new List<IFields>() { dal.CreateIFields().Append("Option", "Update")
    ...
    public void btnVoid_Click()
    {
        ...
        if (!VoidCheckAC.CheckisAC("DM", hidSeed.Text))
        {
        ...
        if (seedid != "")
        {
            ...
            {
                ControlBinder.pageTitleMsg(false, "Domestic Shipment:" + labLotNo.Text, "<p class=\"error\"> Status : Saving  failed  ! ! !   </p>", div_bottom);
            }
            DataBinder();
            ControlBinder.pageTitleMsg(true, "Domestic Shipment:" + labLotNo.Text, "<p>Status :  Edit  Domestic Shipment  of <span>" + labLotNo.Text + "</span></p>", div_bottom);
        }
    }

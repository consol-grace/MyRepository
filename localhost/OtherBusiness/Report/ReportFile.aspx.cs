...
public partial class Triangle_Report_ReportFile : System.Web.UI.Page
{
    ...
    private string[] GetNums(string flag)
    {
        ...
        {
            if (hidsys.Text == "DM" || hidsys.Text == "TK" || hidsys.Text == "BK")
            {
                nums = new string[7];
                //nums[0] = Server.MapPath("~/Report/OtherReportFile/" + hidtype.Text + ".rpt");
    ...
    private void GetPrintMargin()
    {
        DataSet ds = dal.FactoryDAL(PageHelper.ConnectionStrings, "FW_BasicData_Print_SP", new List<IFields>() { dal.CreateIFields().Append("Option", "PrintMargin")
                 .Append("prt_STAT",FSecurityHelper.CurrentUserDataGET()[12]).Append("prt_ReportCode",Request.QueryString["type"]).Append("prt_sys",hidsys.Text).Append("prt_PrinterName",CmbPrint.SelectedItem.Value)
        }).GetList();
        if (ds != null && ds.Tables[0].Rows.Count > 0)
        {

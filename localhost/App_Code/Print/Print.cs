...
public class Print
{
    ...
    /// </summary>
    /// <returns></returns>
    public DataSet GetList(string paperSize, string paperType)
    {
        DataSet ds = dal.FactoryDAL(PageHelper.ConnectionStrings, "FW_BasicData_VirtualPrinter_SP", new List<IFields>() { dal.CreateIFields().
                     Append("option", "PrinterList").Append("PaperSize",paperSize).Append("fileType",paperType)}).GetList();
        return ds;
    }
    ...
    /// </summary>
    /// <param name="printName"></param>
    public void GetPrintName(ref string printName, string paperSize, string paperType)
    {
        int count = 0;
        printName = null;
        ...
        while (string.IsNullOrEmpty(printName))
        {
            ...
            DataSet ds = GetList(paperSize, paperType);

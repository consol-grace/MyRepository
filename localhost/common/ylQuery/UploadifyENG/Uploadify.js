    ...
    $("#btnDelete").click(function() {
        ...
        if (chkCount == 0) {
            ...
            if (confirm("确定删除吗？")) {
                ...
                ylQuery.Controller({ regionId: "table-uploadify-list", url: "Uploadify.aspx?option=delete-select" + pVar }, function(data) {
...
function UploadifyDelete(opts) {
    if (confirm("确认删除吗？")) {
        ...
        ylQuery.Controller({ regionId: opts.selector.parentNode.parentNode, url: "Uploadify.aspx?Option=delete" + strParams }, function(data) {
            var json = $.parseJSON(data);
            if (json[0].Option == "false")
                alert("资料保存失败!");
            else {

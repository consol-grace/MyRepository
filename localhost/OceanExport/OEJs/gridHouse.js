...
//    if (e == "yes") {
            ...
            setTimeout(function() { window.open("../OEContainerList/Container.aspx?HBL=" + Request("seed"), "_blank") }, 1000);
//        }
//    });
}
...
function ReturnNull() {
    ...
    else if ($("#cmbLoading").val() == "" || $("#cmbLoading").val() == null) {
        $("#cmbLoading").focus();
        NullMsg("Loading");
        return false;
    }
    else if ($("#cmbPort").val() == "" || $("#cmbPort").val() == null) {
        $("#cmbPort").focus();
        NullMsg("Discharge");
        return false;
    }
    else if ($("#txtOnBoard").val() == "" || $("#txtOnBoard").val() == null) {
        $("#txtOnBoard").focus();
        NullMsg("Onboard");
        return false;
    }
    else if ($("#txtETD").val() == "" || $("#txtETD").val() == null) {
        $("#txtETD").focus();
        NullMsg("ETD");
        return false;
    }
    else if (HBL == "" || HBL == undefined) {
        if ($("#hidLotNo").val() == "" || $("#hidLotNo").val() == undefined) {
            CompanyX.ShowHbl("2");
            return false;
        }
        else {
    ...
    else if ($("#hidLotNo").val() == "" || $("#hidLotNo").val() == undefined) {
        if (isTran == "TRUE") {
            return true;
        } else {
            CompanyX.ShowHbl("3");
...
function DeleteContainer(grid, rowIndex) {
    ...
    for (var i = 0; i < grid.store.getTotalCount(); ++i) {
        ...
        piece += parseFloat((data.PKG == ""||data.PKG==null) ? 0 : data.PKG);
        status = 1;
    }
    if (status == 1) {

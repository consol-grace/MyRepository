...
function Add() {
    ...
    setTimeout(function () { window.open("../OceanShipmentJobList/OceanShipmentHouse.aspx?MBL=" + Request("seed"), "_blank") }, 1000);
    //        }
    //    });
}
...
function ReturnNull() {
    ...
    onboard = $("#txtOnBoard").val();
    etd = $("#txtETD").val();

    if (type == "" || type == undefined) {
        CmbGroup.focus();
        NullMsg("Service Type");
        return false;
    }
    ...
    else if (onboard == "" || onboard == undefined) {
        txtOnBoard.focus();
        NullMsg("Onboard");
        return false;
    }
...
function getshowVessel(vessel, voyage, controlID) {
    if (controlID == "cmbpreVessel") {
        cmbpreVessel.setValue(vessel);
        CompanyX.BindPreVoyag(voyage);
    }
    else {
...
function ShowVoyage(vesselID, voyageID, controlID) {
    //alert(vessel + "," + voyage);

    var option = "?vesselID=" + vesselID + "&voyageID=" + voyageID + "&controlID=" + controlID;
    win = new Ext.Window({
        id: "Window1",
        title: "Vessel/Voyage",
        resizable: false,
...
function DeleteHBL(grid) {
    ...
    for (var i = 0; i < grid.store.getTotalCount() ; ++i) {
        ...
        gwt += parseFloat((data.o_WT == null || data.o_WT == "") ? 0 : data.o_WT);
        cbm += parseFloat((data.o_CBM == null || data.o_CBM == "") ? 0 : data.o_CBM);
        wm += parseFloat((data.o_WM == null || data.o_WM == "") ? 0 : data.o_WM);
        piece += parseFloat((data.o_PKGS == null || data.o_PKGS == "") ? 0 : data.o_PKGS);
        status = 1;
    }
    if (status == 1) {
        txtAGWT.setValue(gwt == 0 ? "" : gwt);
        txtACBM.setValue(cbm == 0 ? "" : cbm);
        txtAWM.setValue(wm == 0 ? "" : wm);
        txtAPiece.setValue(piece == 0 ? "" : piece);
        totalGWT.setText(gwt == 0 ? "" : Number(gwt).toFixed(3));
        totalCBM.setText(cbm == 0 ? "" : Number(cbm).toFixed(3));
        totalPiece.setText(piece == 0 ? "" : (piece).toFixed(0));
    }
    else {
        txtAGWT.setValue("");
        txtACBM.setValue("");

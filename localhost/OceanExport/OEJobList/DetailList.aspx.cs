...
public partial class OceanExport_OEJobList_DetailList : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!X.IsAjaxRequest)
        {
            if (!string.IsNullOrEmpty(Request["ID"]))
            {
                ...
                else if (Request["type"] == "Container" || Request["type"] == "OEHBL")
    ...
    {
        string s = "";
        DataTable dt = dal.FactoryDAL(PageHelper.ConnectionStrings, "FW_CallShipmentForOE_SP", new List<IFields>() { dal.CreateIFields()

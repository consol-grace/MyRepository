...
public partial class AirExport_AEReportFile_ReportFile : System.Web.UI.Page
{
    ...
    private void LoadValue()
    {
        ...
        DataSet ds = dal.FactoryDAL(PageHelper.ConnectionStrings, "FW_BasicData_Print_SP", new List<IFields>() { dal.CreateIFields().Append("Option", "PrintList")
                 .Append("prt_STAT",FSecurityHelper.CurrentUserDataGET()[12]).Append("prt_ReportCode",Request.QueryString["type"]).Append("prt_sys","AE")
        }).GetList();
    ...
    private string[] GetNums(string flag)
    {
        ...
        {
            nums = new string[7];
            //nums[0] = Server.MapPath("~/Report/AEReportFile/" + hidtype.Text + ".rpt");
        ...
        else if (hidtype.Text == "ShippingNote")
        {
            nums = new string[5];
            nums[0] = Server.MapPath("~/Report/AEReportFile/" + hidtype.Text + ".rpt");
            nums[1] = flag;
        ...
        else if (hidtype.Text == "Manifest")
        {
            ...
            nums[1] = flag;
            nums[2] = CmbPrint.SelectedItem.Value;
            nums[3] = hidID.Text; //air_seed
            nums[4] = hidID.Text; //air_tomawb
            nums[5] = hidID.Text; //air_seed
        }
        else if (hidtype.Text == "Pre-Alert")
        {
            nums = new string[6];
            nums[0] = Server.MapPath("~/Report/AEReportFile/" + hidtype.Text + ".rpt");
            nums[1] = flag;
        ...
        else if (hidtype.Text == "HAWB")
        {
            nums = new string[4];
            nums[0] = Server.MapPath("~/Report/AEReportFile/" + hidtype.Text + "-PROC.rpt");
            nums[1] = flag;
            nums[2] = CmbPrint.SelectedItem.Value;
            nums[3] = hidID.Text;
        }
        else if (hidtype.Text == "HAWB1")
        {
            nums = new string[4];
            nums[0] = Server.MapPath("~/Report/AEReportFile/" + "HAWB" + "View.rpt");
            nums[1] = flag;
            nums[2] = CmbPrint.SelectedItem.Value;
            nums[3] = hidID.Text;
        }
        else if (hidtype.Text == "MAWB")
        {
            nums = new string[4];
            nums[0] = Server.MapPath("~/Report/AEReportFile/" + hidtype.Text + "-PROC.rpt");
            nums[1] = flag;
    ...
    protected void LinBtnPDF_Click(object sender, EventArgs e)
    {
        ...
        //                    Append("inv_seed",hidID.Text).
        //                    Append("code",FSecurityHelper.CurrentUserDataGET()[4]+"AE").
    ...
    private void GetPrintMargin()
    {
        DataSet ds = dal.FactoryDAL(PageHelper.ConnectionStrings, "FW_BasicData_Print_SP", new List<IFields>() { dal.CreateIFields().Append("Option", "PrintMargin")
                 .Append("prt_STAT",FSecurityHelper.CurrentUserDataGET()[12]).Append("prt_ReportCode",Request.QueryString["type"]).Append("prt_sys","AE").Append("prt_PrinterName",CmbPrint.SelectedItem.Value)
        }).GetList();
        if (ds != null && ds.Tables[0].Rows.Count > 0)
        {
    ...
    protected void btnInvDraft_Click(object sender, EventArgs e)
    {
        ...
        Session["Tishi"] = hidID.Text;
        lbltishi.Text = "Printed on  " + DateTime.Now.ToString("dd/MM/yyyy HH:mm").Replace("-", "/");
        ControlBinder.PrintLog(FSecurityHelper.CurrentUserDataGET()[0], FSecurityHelper.CurrentUserDataGET()[12], "AE", hidtype.Text + "(ID:" + hidID.Text + ")", CmbPrint.SelectedItem.Value);
    }
